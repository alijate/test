...
if (isset($_POST['btnConnexion'])) {
    $erreur = fdl_traitement_connexion();
}
else{
    $erreur = false;
}
...
fd_entete('Connexion', '../styles/Digit.css');
...
*
*  @param booléen    $erreur
*/
function fdl_contenu($erreur) {
    ...
    if ($erreur){
        ...
        // $_SERVER['HTTP_REFERER'] n'est pas défini quand l'utilisateur saisit directement l'url
        // de la page dans la barre d'adresse
    ...
    echo
        '<section>',
            ...
            '<form action="connexion.php" method="post">',
                ($erreur ? '<p class="erreur">&Eacute;chec d\'authentification. Utilisateur inconnu ou mot de passe incorrect.</p>' : ''),
                '<table>',
                    '<tr>', 
            ...
            '</form>',
            '<p>Pas encore inscrit ? N\'attendez pas, <a href="inscription.php">inscrivez-vous au service</a> !</p>', 
        '</section>';
}
...
*  @return true    si une erreur a eu lieu.
*/
function fdl_traitement_connexion() {
    ...
    et donc entraînent l'appel de la fonction fd_exit_session() */
    
    if (!fd_controle_parametres('post', array('login', 'password', 'btnConnexion', 'redirection'))){
        fd_exit_session();
    }
